...
import { formatDate, formatDateHuman, getDayStart, parseDate } from '@/lib/date';
...
const TABLE_HEADERS = ['DATE', 'SESSIONS', 'INPUT', 'OUTPUT', 'CACHE', 'COST'];
...
const dailyCommand = define({
	...
	run: async (ctx) => {
		...
		const sessions = await aggregateSessions(messagesDir, configs, { silent: isSilent });
		...
		const summary = aggregateByPeriod(filtered, (date) => formatDate(getDayStart(date).getTime()));
		...
		if (outputFormat === 'json') {
			const jsonRows = periodRows.map((row) => {
				const parsed = parseDate(row.period);
				const periodLabel = parsed ? formatDateHuman(parsed.getTime()) : row.period;
			...
			console.log(formatPeriodAsJson(jsonRows, 'daily'));
			return;
		}
