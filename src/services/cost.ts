...
const toModelConfig = (record: Record<string, unknown>): ModelConfig | null => {
  ...
  const contextWindow = getNumberValue(record, [
    "contextWindow",
    "context_window",
  ]);
...
const findMatchingModel = (
  ...
  for (const candidate of candidates) {
    const match = indexedConfigs.find(
      (entry) =>
        entry.normalizedNoDate === candidate ||
        entry.normalizedNoDate.startsWith(`${candidate}-`),
    );
    if (match) {
  ...
  if (normalizeVersionSeparators(baseId).startsWith("gpt-")) {
    const fallbackMatch = indexedConfigs.find(
      (entry) =>
        entry.normalizedNoDate === "gpt-4o" ||
        entry.normalizedNoDate.startsWith("gpt-4o-"),
    );
    if (fallbackMatch) {
...
export const loadModelConfigs = async (
  ...
  if (!exists) {
    logWarning(
      `Model config not found at ${modelsFile} (all costs set to 0)`,
      options,
    );
    return {};
  }
  ...
  try {
    ...
    logWarning(
      `Failed to parse model config at ${modelsFile} (all costs set to 0)`,
      options,
    );
  ...
  if (!isRecord(data)) {
    logWarning(
      `Invalid model config format at ${modelsFile} (all costs set to 0)`,
      options,
    );
    return {};
  }
...
export const loadAllModelConfigs = async (
  ...
  logWarning(
    `Model config not found at ${modelsPath} (all costs set to 0)`,
    options,
  );
  return {};
};
...
export const calculateMessageCost = (
  ...
  return calculateUsageCost(
    normalizeTokens(message.tokens),
    message.modelID,
    configs,
    options,
  );
