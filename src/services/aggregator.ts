...
export const aggregateByModel = (sessions: Session[]): Map<string, UsageSummary> => {
...
export const aggregateByPeriod = (
	...
	for (const session of sessions) {
		...
		if (!current) {
			...
			for (const [modelId, usage] of Object.entries(session.models)) {
		...
		for (const [modelId, usage] of Object.entries(session.models)) {
			...
			if (existing) {
				current.models[modelId] = {
					...
					cacheTokens: existing.cacheTokens + usage.cacheTokens,
					costUSD: existing.costUSD + usage.costUSD,
				};
				continue;
			}
			current.models[modelId] = { ...usage };
		}
