...
const resolveLogLevel = (override?: AppConfig['logLevel']): AppConfig['logLevel'] => {
	...
	if (isNonEmpty(envLevel)) {
		consola.warn(`Unknown log level: ${envLevel} (fallback to ${DEFAULT_CONFIG.logLevel})`);
	}
...
export const loadAppConfig = (overrides: Partial<AppConfig> = {}): AppConfig => {
