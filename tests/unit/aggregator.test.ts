...
describe('aggregator', () => {
	...
	test('aggregateByPeriod groups sessions by period key', () => {
		...
		const summary = aggregateByPeriod([sessionA, sessionB], (date) => formatDate(date.getTime()));
		...
		expect(modelNames).toEqual(['model-a', 'model-b']);
		expect(entry?.models['model-a']?.inputTokens).toBe(1);
		expect(entry?.models['model-b']?.inputTokens).toBe(2);
	});
